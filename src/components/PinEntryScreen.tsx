...
export function PinEntryScreen({ onSuccess, onLogout }: PinEntryScreenProps) {
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    ...
    // We enforce 4-6 digits. Wait, let's just do an exact match check.
    const expectedPin = selectedUser.pin_code || '0000';
    if (pin === expectedPin) {
      if (!selectedUser.active) {
        setError('هذا الحساب غير نشط. يرجى مراجعة مدير النظام.');
        return;
      }
          ...
          {!selectedUser ? (
            <div className="space-y-6">
              <div className="text-center space-y-2">
                ...
                <h2 className="text-2xl font-bold text-surface-900">مرحباً بك</h2>
