      ...
      <Card className="border-0 shadow-sm">
         <CardBody className="p-6">
            <div className="flex flex-wrap items-end gap-4 mb-8">
               <div>
                  <label className="block text-sm font-bold text-surface-700 mb-1">نوع التقرير</label>
               ...
               <Button variant="outline" className="border-primary-200 text-primary-700 hover:bg-primary-50 font-bold">
            ...
            {reportType === 'daily' && (
              <div className="space-y-6">
                <div className="flex items-center gap-2 mb-2 text-surface-800">
                  <Activity className="w-6 h-6 text-emerald-600" />
                  <h3 className="text-lg font-black">المبيعات لآخر 7 أيام</h3>
                </div>
                <div className="h-[400px] w-full bg-surface-50 border border-surface-100 p-4 rounded-3xl">
            ...
            {reportType === 'fleet' && (
              <div className="bg-surface-50 border border-surface-200 rounded-2xl p-6">
                 <div className="flex items-center gap-2 mb-6 text-surface-800">
                 ...
                 {fleetData.length > 0 ? (
                   <div className="overflow-x-auto">
                     <table className="w-full text-right border-collapse">
                       <thead>
                         <tr className="border-b-2 border-surface-200 text-surface-500 text-sm">
                       ...
                       <tbody>
                         {fleetData.map((v, i) => {
                           ...
                           return (
                             <tr key={i} className="border-b border-surface-100 hover:bg-white transition-colors">
                               <td className="py-3 px-4 font-bold text-surface-800">{v.name || 'سيارة'} ({v.plate_number || '-'})</td>
                               <td className="py-3 px-4 text-surface-600">{v.type === 'motorcycle' ? 'دباب' : v.type === 'van' ? 'عربة' : 'سيارة'}</td>
                               <td className="py-3 px-4 font-bold text-primary-700">{v.worker_name || 'غير محدد'}</td>
                               <td className="py-3 px-4 text-emerald-600 font-bold">{washed} سيارة</td>
                               <td className="py-3 px-4 font-black text-surface-700">{income} ريال</td>
                             </tr>
                           )
                         })}
                       </tbody>
